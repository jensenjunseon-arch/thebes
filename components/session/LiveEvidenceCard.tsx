...
export function LiveEvidenceCard({ evidence }: { evidence: LiveEvidence | null }) {
  ...
  return (
    <div
      key={evidence.quote}
      className="animate-evidence-in rounded-2xl border border-accent/30 bg-accent-soft/40 p-4"
    >
      <p className="font-mono text-[11px] uppercase tracking-tighter2 text-ink/45">
        AI가 방금 감지한 것
      </p>

      <blockquote className="mt-2 border-l-2 border-accent pl-3 text-[15px] leading-relaxed text-ink/85">
        “{evidence.quote}”
      </blockquote>
